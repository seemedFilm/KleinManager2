/**
 * adBuilder Integration Module - FIXED VERSION
 * Version: 1.0.3 
 * 
 * FIX: Button wird ÜBER Settings eingefügt (nicht unten im Footer)
 */
...
(function() {
    ...
    window.adDebug = {
        ...
        help: function() {
            console.clear();
            console.log('%c📚 adBuilder Debug Helper v1.0.3', 'font-size: 18px; font-weight: bold; color: #667eea;');
...
Beispiel:
  ...
  3. adDebug.testButton()    // Schritt 3
    ...
    function initAdBuilder() {
        ...
        // Versuche einzufügen - WICHTIG: VOR Settings, nicht nach!
    ...
    function insertButton(sidebar, btn, config) {
        // FIX v1.0.3: Button soll ÜBER Settings sein, nicht unten!

        // Strategie 1: Finde Settings-Button und füge DAVOR ein
        const settingsBtn = findSettingsButton(sidebar);
        if (settingsBtn && settingsBtn.parentNode) {
            adDebug.log('📌 Strategie 1: Füge VOR Settings-Button ein', 'info');
            settingsBtn.parentNode.insertBefore(btn, settingsBtn);
            adDebug.log('✅ Button VOR Settings eingefügt!', 'success');
            return true;
        }

        // Strategie 2: Füge am Anfang eines Button-Containers ein
        try {
            adDebug.log('📌 Strategie 2: Füge am Anfang ein', 'info');
            const firstButton = sidebar.querySelector('button');
            if (firstButton) {
                firstButton.parentNode.insertBefore(btn, firstButton);
                adDebug.log('✅ Button am Anfang eingefügt!', 'success');
                return true;
            }
        } catch (e) {
            adDebug.log(`Strategie 2 fehlgeschlagen: ${e.message}`, 'warning');
        }

        // Strategie 3: Einfach am Anfang der Sidebar anhängen
        try {
            adDebug.log('📌 Strategie 3: Am Anfang der Sidebar', 'info');
            sidebar.insertBefore(btn, sidebar.firstChild);
            adDebug.log('✅ Button am Anfang der Sidebar eingefügt!', 'success');
            return true;
        } catch (e) {
            adDebug.log(`Strategie 3 fehlgeschlagen: ${e.message}`, 'warning');
        }

        // Fallback: Am Ende anhängen
        try {
            adDebug.log('📌 Strategie 4: Am Ende anhängen (Fallback)', 'warning');
            sidebar.appendChild(btn);
            adDebug.log('⚠️ Button am Ende eingefügt (nicht ideal)', 'warning');
            return true;
        } catch (e) {
            adDebug.log(`Strategie 4 fehlgeschlagen: ${e.message}`, 'error');
            return false;
        }
    }

    /**
     * Finde Settings-Button mit mehreren Strategien
     */
    function findSettingsButton(sidebar) {
        ...
        for (const selector of selectors) {
            try {
                ...
                if (element) {
                    adDebug.log(`✅ Settings-Button gefunden: "${element.textContent.trim()}"`, 'success');
                    return element;
                }
            } catch (e) {
                // Selector-Fehler (z.B. :contains ist nicht standard)
            }
        }

        // Fallback: Suche nach Button mit "Settings" im Text
        const buttons = sidebar.querySelectorAll('button');
        for (const btn of buttons) {
            if (btn.textContent.toLowerCase().includes('settings') || 
                btn.textContent.toLowerCase().includes('einstellungen')) {
                adDebug.log(`✅ Settings-Button per Text-Match gefunden: "${btn.textContent.trim()}"`, 'success');
                return btn;
            }
        }

        adDebug.log('⚠️ Settings-Button nicht gefunden', 'warning');
        return null;
    }
    ...
    // Export APIs
    window.AdBuilderIntegration = { init: initAdBuilder, version: '1.0.3' };
})();

// Automatisch Hilfe anzeigen beim Load
console.log('%c🎉 adBuilder v1.0.3 geladen!', 'font-size: 16px; font-weight: bold; color: #2ecc71;');
